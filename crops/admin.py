...
from .models import GardenArea, Bed, Plot, VegetableFamily, VegetableType, Crop, SoilStatusArea,MaintenanceLog
import datetime
...
class GardenAreaAdmin(admin.ModelAdmin):
    ...
    def grid_link(self, obj):
        # layout/ に飛ばすように変更
        return mark_safe(f'<a class="button" href="{obj.pk}/layout/">レイアウト編集</a> '
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('<int:pk>/layout/', self.admin_site.admin_view(self.layout_view), name='gardenarea-layout'),
            path('<int:pk>/manage/', self.admin_site.admin_view(self.manage_view), name='gardenarea-manage'),
            path('<int:pk>/assign-range/', self.admin_site.admin_view(self.assign_range_view), name='assign-grid-range'),
            path('<int:pk>/plant-crop/', self.admin_site.admin_view(self.plant_crop_view), name='plant-crop'),
            path('<int:pk>/assign-soil-status/', self.admin_site.admin_view(self.assign_soil_status_view), name='assign-soil-status'),
            path('<int:pk>/get-logs/<int:plot_id>/', self.admin_site.admin_view(self.get_plot_logs), name='plot-logs'),
            path('<int:pk>/save-maintenance/', self.admin_site.admin_view(self.save_maintenance), name='save-maintenance'),
        ]
    ...
    def get_plot_logs(self, request, plot_id, **kwargs):
        ...
        logs = MaintenanceLog.objects.filter(plots__id=plot_id).order_by('-worked_at')[:3]
    ...
    def get_daily_logs(self, instance, selected_date):
        ...
        logs = MaintenanceLog.objects.filter(area=instance,worked_at__date=selected_date)
        log_list = []
        for log in logs:
            log_list.append({
                'worked_at': log.worked_at.strftime('%m/%d %H:%M'),
                'task_display': log.get_task_type_display(),
                'plots': [f"[{p.row_index}-{p.col_index}]" for p in log.plots.all()],
                'user': log.user.username if log.user else "不明",
                'note': log.note
            })
    ...
    def get_common_grid_data(self, instance, selected_date):
        # 1. 全マスのID対応表
        all_plots_qs = Plot.objects.filter(area=instance).values_list('row_index', 'col_index', 'id')
        all_plot_ids_dict = {f"{r}-{c}": pid for r, c, pid in all_plots_qs}

        # 2. 指定日にアクティブな情報を取得
        active_beds = instance.beds.filter(
        ...
        logged_plot_ids = MaintenanceLog.objects.filter(
            area=instance, 
            worked_at__date=selected_date
        ).values_list('plots__id', flat=True)

        active_plots = Plot.objects.filter(
            area=instance
        ).filter(
        ...
        ).distinct().select_related('crop_here__vegetable_type').prefetch_related('beds')
        ...
        for p in active_plots:
            ...
            try:
                c = p.crop_here
                if c.planted_at <= selected_date and (c.harvested_at is None or c.harvested_at >= selected_date):
                    current_crop = c
            except:
                pass
            ...
            plot_dict[key] = {
                ...
                "crop_icon_url": current_crop.vegetable_type.icon.url if current_crop and current_crop.vegetable_type.icon else "",
    ...
    def assign_range_view(self, request, pk):
        if request.method == 'POST':
            ...
            if not plot_ids:
                return JsonResponse({'status': 'error', 'message': 'マスが選択されていません'})

            if mode == 'create_bed':
                new_name = data.get('bed_name') or f"畝_{timezone.now().strftime('%m%d_%H%M')}"
                ...
                return JsonResponse({'status': 'ok', 'message': f'新しい畝「{new_bed.name}」を登録しました'})

            elif mode == 'clear':
                plots = Plot.objects.filter(id__in=plot_ids)
                for p in plots:
                    p.beds.clear()
                return JsonResponse({'status': 'ok', 'message': '選択範囲の畝解除が完了しました'})
    ...
    def assign_soil_status_view(self, request, pk):
        if request.method == 'POST':
            ...
            plot_ids = data.get('plot_ids', [])
            print(f"Received soil status assignment: {status_type} starting from {start_date} for plots {plot_ids}")
    ...
    def plant_crop_view(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            if not plot_ids:
                self.message_user(request, "対象のマスが指定されていません", level='error')
            ...
            if mode == 'delete':
                ...
                for plot in plots:
                    if hasattr(plot, 'crop_here'):
                        name = plot.crop_here.vegetable_type.name if plot.crop_here.vegetable_type else "不明"
                        deleted_names.append(name)
                        plot.crop_here.delete()
                if deleted_names:
                    self.message_user(request, f"{', '.join(deleted_names)} を削除しました")
                else:
                ...
                if not type_id or not planted_at:
                    self.message_user(request, "野菜の種類と植付日を選択してください", level='error')
                    return HttpResponseRedirect('../grid/')
...
class PlotAdmin(admin.ModelAdmin):
    list_display = ('area', 'row_index', 'col_index', 'get_beds', 'get_crop')
    list_filter = ('area',)
    ...
    def get_beds(self, obj):
        beds = obj.beds.all()
        return ", ".join([b.name for b in beds]) if beds else "-"
    get_beds.short_description = "所属する畝"

    def get_crop(self, obj):
        # [修正] crop_here.name → crop_here.vegetable_type.name
        if hasattr(obj, 'crop_here') and obj.crop_here.vegetable_type:
            return obj.crop_here.vegetable_type.name
        return "-"
    get_crop.short_description = "作付け"
    ...
    def assign_plots_to_bed(self, request, queryset):
        if 'apply' in request.POST:
            ...
            if form.is_valid():
                ...
                self.message_user(request, f"{updated_count} 個のマスを「{bed.name}」に割り当てました。")
                return HttpResponseRedirect(request.get_full_path())

        initial_data = {'_selected_action': request.POST.getlist(helpers.ACTION_CHECKBOX_NAME)}
        if queryset.exists():
            initial_data.update({
        ...
        return render(request, 'admin/crops/assign_bed_range.html', {
            'items': queryset,
            'form': form,
            'title': u'範囲を指定して畝を割り当てる'
        })
...
class VegetableTypeAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('import/', self.admin_site.admin_view(self.import_view), name='crops_vegetabletype_import'),
        ]
        return custom_urls + urls
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                    ...
                    try:
                        with zipfile.ZipFile(zip_file) as zf:
                            for info in zf.infolist():
                                if not info.is_dir():
                                    zip_files[os.path.basename(info.filename)] = zf.read(info)
                    except zipfile.BadZipFile:
                        form.add_error('zip_file', '有効な ZIP ファイルを指定してください。')
                        return render(request, 'admin/crops/vegetabletype_import.html', {
                            **self.admin_site.each_context(request),
                            'form': form,
                ...
                for row_num, row in enumerate(reader, start=2):
                    ...
                    spacing_value = (row.get('spacing_cm') or row.get('spacing') or '').strip()
                    icon_filename = (row.get('icon_filename') or row.get('icon') or '').strip()
                    ...
                    try:
                        ...
                        errors.append(f'行 {row_num}: spacing_cm は整数である必要があります。')
                        continue
                    ...
                    if icon_filename:
                        ...
                        if icon_data:
                            vt.icon.save(os.path.basename(icon_filename), ContentFile(icon_data), save=False)
                        else:
                            errors.append(f'行 {row_num}: icon_filename {icon_filename} が ZIP に見つかりません。')
                ...
                self.message_user(request, f'インポート完了: {created_count} 件作成, {updated_count} 件更新。')
                if errors:
                    for error in errors:
...
class CropAdmin(admin.ModelAdmin):
    list_display = ('id', 'get_vegetable_name', 'family_display', 'planted_at', 'main_plot')
